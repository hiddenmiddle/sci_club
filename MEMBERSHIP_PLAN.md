...
1. Открой **Product catalog**, зайди в уже созданный продукт членства клуба и в блоке Pricing скопируй **Price ID** (начинается с `price_…`) — пришли его. Заодно проверь, что цена стоит как **$15.00 USD, recurring / monthly**.
2. Переключись в **Test mode** (тумблер сверху справа). Каталог продуктов там отдельный, поэтому продукта скорее всего нет — создай такой же (Add product, $15.00 USD, recurring / monthly) и пришли **тестовый Price ID**.
3. Developers → API keys: пришли **Secret key** из live-режима (`sk_live_…`) и из test-режима (`sk_test_…`).
...
**И ещё одно:**
**Ссылка-приглашение в Telegram-группу** клуба (invite link) — её будем автоматически отправлять оплатившим.

Пароль от админ-панели придумает Алекс и пришлёт тебе.

Итого прислать: live Price ID, test Price ID, `sk_live_…`, `sk_test_…`, адрес Gmail + app password, Telegram invite link.
